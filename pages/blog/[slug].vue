...
const { data: surround } = await useAsyncData(`${route.path}-surround`, () => queryContent("/blog").where({ _extension: "md" }).without(["body", "excerpt"]).sort({ date: -1 }).findSurround(withoutTrailingSlash(route.path)), { default: () => [] });
